...
import AutomaticPath from "./automaticPath.js";
...
export default class ThumbRaiser {
  ...
  constructor(generalParameters, audioParameters, cubeTexturesParameters, mazeParameters, playerParameters, ambientLightParameters, directionalLightParameters, spotLightParameters, flashLightParameters, shadowsParameters, fogParameters, collisionDetectionParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters, myPathArray) {
    ...
    *  [5,5]
    *
    *   this.myPathArray = [[3,0], [4,4], [5,5]];
    *
    * */
      // this.myPathArray2 = ["A1", [3,0], [4,4], [5,5]];     cel("A1", [-8.5,0, -1.5], [10.9,0])
      // this.myPathArray2 = ["A2", [4,0], [4,3], [5,5]];     cel(a,a1,[2,3],[4,5])

    /*this.myPathArray = {
      // "A1": [new THREE.Vector3(-8.5, 0, -1.5), new THREE.Vector3(10.9, 0, 0.8)],    // passageway
      "A1": [new THREE.Vector3(-8.5, 0, -1.5), new THREE.Vector3(10.5, 0, -2.9)],
      "A2": [new THREE.Vector3(-3.5, 0, -1.5), new THREE.Vector3(5, 0, 0.3)],
      "B4": [new THREE.Vector3(0, 0, 0)]
    };*/
    ...
    this.passagewayBoolean= false;
    this.locationExit = null;

    this.flagAutomaticPathElevator = false;
    this.maze = new Maze(this.mazeParameters, this.firstPersonViewCamera, this.scene, false, this, this.passagewayBoolean, this.locationExit, this.automaticPath, this.flagAutomaticPathElevator);
    // Caminho Automático
    //this.maze.flagAutomaticPathElevator = this.flagAutomaticPathElevator;
    this.loadPathData('assets/mazes/arrayPathData.json').then(pathArray => {
      this.myPathArray = pathArray;
      console.log(this.myPathArray);
      // Inicializar o AutomaticPath com os novos dados do caminho
      this.automaticPath = new AutomaticPath(this.player, this.myPathArray, 0.01, this.maze, this.userInterface);
    });



    // this.automaticPath = new AutomaticPath(this.player, myPathArray, 0.01, this.maze, this.userInterface);
  ...
  async loadPathData(url) {
    const response = await fetch(url);
    const data = await response.json();
    for (const key in data) {
      data[key] = data[key].map(coords => new THREE.Vector3(...coords));
    }
    return data;
  }

  updateCurrentMaze(newMaze) {
    this.maze = newMaze;
    this.automaticPath.setMaze(newMaze);
  }
    ...
    if (!this.gameRunning) {
      ...
      if (!this.animations.actionInProgress) {
        ...
        if (this.maze.foundExit(this.player.position)) {
          ...
          if (this.maze.collision(this.collisionDetectionParameters.method, position, this.collisionDetectionParameters.method != "obb-aabb" ? this.player.radius : this.player.halfSize, directionRad - this.player.defaultDirection,this.userInterface, this)) {
      ...
      //this.newMaze = new Maze(this.mazeParameters, this.firstPersonViewCamera, this.scene, false, this, this.passagewayBoolean, this.locationExit);
      //this.updateCurrentMaze(this.newMaze);

      if (this.automaticPath && this.automaticPath.isMoving) {
        this.automaticPath.update();
      }
