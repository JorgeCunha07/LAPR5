...
export default class UserInterface extends GUI {
    ...
    constructor(thumbRaiser) {
        ...
        //console.log("A: " + this.thumbRaiser);
        ...
        // Automatic Path Folder
        // Dentro do construtor da UserInterface
        const automaticPathFolder = this.addFolder("Caminho Automático");
        automaticPathFolder.domElement.style.fontSize = fontSize;
        // Adicionar apenas o botão Start que usa o caminho predefinido
        automaticPathFolder.add({ startPath: () => {
          thumbRaiser.automaticPath.setPaths(thumbRaiser.myPathArray); // Define o caminho pré-definido
          thumbRaiser.automaticPath.start(); // Inicia o movimento ao longo do caminho
          thumbRaiser.maze.flagAutomaticPathElevator = true;
        }}, 'startPath').name("Start");
      automaticPathFolder.add({ stopPath: () => {
          thumbRaiser.automaticPath.stop(); // Para o movimento automático e reativa o controle manual
          thumbRaiser.maze.flagAutomaticPathElevator = false;
        }}, 'stopPath').name("Stop");
      ...
      const mapFolder = this.addFolder("Map");
      const mapOptions = [
        "A1",
        "A2",
        "B1",
      ...
      mapFolder
        .add(defaultMapOption, "Map", mapOptions, defaultMapOption)
        .name("Select Map")
        .onChange((event) => {
          fetch(`assets/mazes/${event}.json`)
            .then((res) => {
              return res.json();
            })
            .then((data) => {
              ...
              thumbRaiser.maze.loaded = false;
              thumbRaiser.gameRunning = false;
              thumbRaiser.mazeParameters = { url: `assets/mazes/${event}.json`};
              thumbRaiser.firstTimeOpening = true;
              let helperAutomaticPath = thumbRaiser.maze.flagAutomaticPathElevator;
              if (thumbRaiser.maze.flagAutomaticPathElevator == null){
                helperAutomaticPath = false;
              }
              //thumbRaiser.maze = new Maze(thumbRaiser.mazeParameters, thumbRaiser.firstPersonViewCamera, thumbRaiser.scene, false, thumbRaiser,true,nearestPassageway.locationOnExit,thumbRaiser.maze.automaticPath, helperAutomaticPath);
              thumbRaiser.maze = new Maze(thumbRaiser.mazeParameters, thumbRaiser.firstPersonViewCamera, thumbRaiser.scene, false, thumbRaiser,false,null,thumbRaiser.maze.automaticPath, helperAutomaticPath);
              thumbRaiser.updateCurrentMaze(thumbRaiser.maze);
  ...
  loadMapFromURL(mapName) {
    const thumbRaiser = this.thumbRaiser;
    const url = `assets/mazes/${mapName}.json`;
    fetch(url)
      .then(res => res.json())
      .then(data => {
        console.log("valor recebido a alterar");
        console.log(thumbRaiser.maze.flagAutomaticPathElevator);
        this.flagAutomaticPathElevator = thumbRaiser.maze.flagAutomaticPathElevator;
        thumbRaiser.scene.remove(thumbRaiser.maze);
        thumbRaiser.maze.loaded = false;
        thumbRaiser.gameRunning = false;
        thumbRaiser.mazeParameters = { url: url };
        thumbRaiser.firstTimeOpening = true;

        console.log("valor recebido a alterado");
        console.log(this.flagAutomaticPathElevator);
        if (thumbRaiser.maze.flagAutomaticPathElevator == null){
          this.flagAutomaticPathElevator = false;
        }
        thumbRaiser.maze = new Maze(thumbRaiser.mazeParameters, thumbRaiser.firstPersonViewCamera, thumbRaiser.scene, true, thumbRaiser,false,null,thumbRaiser.maze.automaticPath,this.flagAutomaticPathElevator);
        thumbRaiser.updateCurrentMaze(thumbRaiser.maze);
        thumbRaiser.scene.add(thumbRaiser.maze);
  ...
  loadMapFromURLPassageway(nearestPassageway) {

    const mapName = nearestPassageway.floor;
    const thumbRaiser = this.thumbRaiser;
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        thumbRaiser.mazeParameters = { url: url };
        thumbRaiser.firstTimeOpening = true;
        thumbRaiser.maze = new Maze(thumbRaiser.mazeParameters, thumbRaiser.firstPersonViewCamera, thumbRaiser.scene, false, thumbRaiser,true,nearestPassageway.locationOnExit,thumbRaiser.maze.automaticPath, this.flagAutomaticPathElevator);
        thumbRaiser.updateCurrentMaze(thumbRaiser.maze);
        thumbRaiser.scene.add(thumbRaiser.maze);
  ...
  createLoadingScreen() {
    ...
    loadingImage.src = 'assets/loading/elevatorLoadingScreen.png';
    loadingImage.alt = 'Loading';
    loadingImage.style.width = '100%';
    loadingImage.style.height = '100%';
    loadingImage.style.objectFit = 'cover'; // Manter proporções

    loadingScreen.appendChild(loadingImage);
    document.body.appendChild(loadingScreen);
  }

  createPassagewayLoadingScreen() {
    const loadingScreenPassageway = document.createElement('div');
    loadingScreenPassageway.id = 'loadingScreenPassageway';
    loadingScreenPassageway.style.position = 'fixed';
    loadingScreenPassageway.style.top = '0';
    loadingScreenPassageway.style.left = '0';
    loadingScreenPassageway.style.width = '100%';
    loadingScreenPassageway.style.height = '100%';
    loadingScreenPassageway.style.backgroundColor = 'rgba(0, 0, 0, 0.5)';
    loadingScreenPassageway.style.display = 'flex';
    loadingScreenPassageway.style.justifyContent = 'center';
    loadingScreenPassageway.style.alignItems = 'center';
    loadingScreenPassageway.style.zIndex = '1001';

    const loadingImagePassageway = document.createElement('img');
    loadingImagePassageway.src = 'assets/loading/passagewayLoadingScreen.png';
    loadingImagePassageway.alt = 'Loading';
    loadingImagePassageway.style.width = '100%';
    loadingImagePassageway.style.height = '100%';
    loadingImagePassageway.style.objectFit = 'cover'; // Manter proporções

    loadingScreenPassageway.appendChild(loadingImagePassageway);
    document.body.appendChild(loadingScreenPassageway);
  }
