...
export default class RoleService implements IRoleService {
  ...
  public async getAllRole(): Promise<Result<Array<IRoleDTO>>> {
    try {

      let listRoles : Result<Array<Role>> = await this.roleRepo.getAllRoles();

      if (listRoles.isFailure) {
        return Result.fail<Array<IRoleDTO>>(listRoles.errorValue());
      }

      const roles = listRoles.getValue();
      const roleDTOs = roles.map(role => RoleMap.toDTO(role)); // Map each Role to an IRoleDTO
